  ...
  void _searchapi(String key) {
    ...
    PreferenceConnector.getJsonToSharedPreferencetoken(StringConstant.loginData)
        .then((value) => {
              if (value != null)
                {
                  ApiRepository().searchusers(key, value).then((value) {
                    ...
                    return InkWell(
                          ...
                          child: Row(
                            ...
                            children: [
                              CircleAvatar(
                                ...
                                backgroundColor: AppColors.appNewDarkThemeColor,
                                child: Center(
                                    child: Text("${users[position].name[0]}",
                        ...
                        onTap: () async {
                          if (widget.page != "InGroup") {
                            peerIdApi(users[position].id, users[position],
                                users[position].name);
                          } else {
                            addGroupUser
                                .add("#" + users[position].id.toString() + "#");
    ...
    PreferenceConnector.getJsonToSharedPreferencetoken(StringConstant.loginData)
        .then((value) => {
              if (value != null)
                {
                  ApiRepository()
                      .addfriend(userdata.id.toString(), value)
                      .then((value) {
                    if (value != null) {
                      if (value.status == "successfull") {
                        ...
                        EasyLoading.showToast(
                            "${addUserName} Added in chat list.",
                            toastPosition: EasyLoadingToastPosition.top,
                            duration: Duration(seconds: 3));
                      } else {
                        EasyLoading.showToast(
                            "${addUserName} already Added in chat list.",
                            toastPosition: EasyLoadingToastPosition.top,
                            duration: Duration(seconds: 3));
                      }
                    }
